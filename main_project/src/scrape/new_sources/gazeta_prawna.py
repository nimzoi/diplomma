"""Gazeta Prawna — sekcja Twoje Prawo (porady prawnicze).

Discovery: kategoria /twoje-prawo + paginacja, plus edgp.gazetaprawna.pl/<sek>/artykuly/N,SLUG.html.
...
logger = logging.getLogger("scrape.new_sources.gazeta_prawna")

BASE = "https://www.gazetaprawna.pl"
EDGP_BASE = "https://edgp.gazetaprawna.pl"
SOURCE_NAME = "gazetaprawna.pl"

# Listing pages on the two domains
LISTING_PAGES_MAIN = [
    "/twoje-prawo",
    "/wiadomosci/prawo",
    "/wiadomosci/podatki",
    "/wiadomosci/finanse-osobiste",
]
LISTING_PAGES_EDGP = [
    "/kraj/spoleczenstwo",
    "/gospodarka",
...
# Article patterns
ARTICLE_RE_MAIN = re.compile(
    r'(https://(?:www|edgp)\.gazetaprawna\.pl/[\w/-]+/artykuly/\d+,[\w-]+\.html|'
    r'/wiadomosci/[\w-]+,\d+\.html)'
)


def discover_article_urls(fetcher: Fetcher) -> list[str]:
    seen: set[str] = set()
    for base, pages in [(BASE, LISTING_PAGES_MAIN), (EDGP_BASE, LISTING_PAGES_EDGP)]:
        for path in pages:
            for pg in range(1, MAX_PAGES + 1):
                url = f"{base}{path}" if pg == 1 else f"{base}{path}/strona-{pg}"
                resp = fetcher.get(url)
                if resp is None or resp.status_code != 200:
                    break
                text = resp.content.decode("utf-8", "replace")
                found = ARTICLE_RE_MAIN.findall(text)
                new = 0
                for href in found:
                    if href.startswith("/"):
                    ...
                    if href not in seen:
                        seen.add(href)
                        new += 1
                logger.info("%s%s page %d: +%d (total %d)", base, path, pg, new, len(seen))
                if new == 0 and pg > 1:
                    break
    return sorted(seen)
...
def parse_article(html: str, url: str, idx: int) -> ArticleRecord | None:
    ...
    # Skip articles that require login (DGP premium)
    if "Aby kontynuować, zaloguj się" in html or "Treść dostępna w prenumeracie" in html:
        return None
    clean_soup(soup)
...
def scrape(output_dir: Path, max_articles: int | None) -> ScrapeSummary:
    ...
    for idx, url in enumerate(urls, start=1):
        ...
        if rec is None:
            ...
            failed.append((url, "too_short_or_premium"))
            continue
        records.append(rec)
    ...
    summary.notes = (
        f"Listing walks on www.gazetaprawna.pl ({len(LISTING_PAGES_MAIN)} sekcji) "
        f"+ edgp.gazetaprawna.pl ({len(LISTING_PAGES_EDGP)}). Premium articles "
        "wykluczone."
    )
    ...
    write_readme(
        ...
        BASE,
        LICENSE_FAIR_USE,
        "Listing pagination + edgp subdomain articles",
    )
    return summary
