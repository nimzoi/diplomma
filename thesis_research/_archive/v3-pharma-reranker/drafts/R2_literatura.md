...
4. **Prace dotyczące czysto cross-language NLP** bez komponentu cross-register — wykluczone, chyba że są bezpośrednim *prior art* dla cross-register medical (np. Grabowski 2018, włączony pomimo cross-language framing).
...
| Author (Year) | Method | Domain | Contribution | Limitation |
|---|---|---|---|---|
| Grabowski (2018) [26] | English-Polish comparable PIL corpus | Cross-language (EN ↔ PL) medical | Pierwszy publicznie udokumentowany PIL corpus EN-PL | Cross-language, nie intra-PL cross-register; brak retrievalu |
...
1. **Polish-specific medical reranker** — polish-reranker-roberta-v3 [10] istnieje jako general-domain baseline; dotrenowanie na medical/pharmaceutical content nie zostało publicznie udokumentowane w przeglądanej literaturze. **Wkład:** RQ1 (iteracyjny retraining rerankera dla polish pharma, z adresowaniem polish-specific patterns wokół międzynarodowej terminologii — fleksja DCI, szyk, regulatorowa frazeologia, refundacja PL).
2. **Walidacja LLM-as-judge dla polish medical domain** — Zheng 2023 [12] i Liu 2023 [13] walidują GPT-4 jako judge dla English general-domain. Bielik [18] / PLLuM [19] **nie zostali zewaluowani jako judge dla polish specialty domain w przeglądanej literaturze**. **Wkład:** RQ2 (walidacja LLM-judge dla polish pharma z kappa ≥0.50).
3. **Polish ChPL↔Ulotka aligned corpus** — Grabowski 2018 [26] dostarcza EN-PL comparable PIL, ale **nie intra-Polish cross-register**. Cao 2020 [27], Devaraj 2021 [28], van den Bercken 2019 [29] adresują expertise style transfer **w języku angielskim** i jako *generation*, nie *retrieval*. Polish ChPL↔Ulotka aligned corpus nie został publicznie udokumentowany w przeglądanej literaturze. **Wkład:** RQ5 (cross-register retrieval ChPL ↔ Ulotka).
4. **Simulated drift framework dla polish retrieval system** — Evidently [24] i Alibi Detect [25] są tool-level, drift datasets publiczne istnieją tylko dla anglojęzycznych domen. Simulated drift framework dla polskich systemów retrievalu nie pojawia się w literaturze polskojęzycznej. **Wkład:** RQ4 (simulated drift detection na polish medical embeddings).
5. **Iteracyjny retraining framework z plateau analysis** dla polish NLP — Sculley 2015 [20] i Treveil 2020 [21] dostarczają framework MLOps, ale aplikacje na polish reranking są empirically niezbadane. Plateau analysis dla iteracyjnego retreningu polskich rerankerów nie zostały empirycznie zbadane w przeglądanej literaturze. **Wkład:** RQ3 (plateau analysis przez 3 cykle retreningu).
...
- [ ] **Rozszerzenie sekcji 2.6** — dodać ~200 słów więcej *empirical* details dla Grabowski 2018 (rozmiar korpusu, methodology, dlaczego konkretnie EN-PL nie wystarcza dla naszego use case).
