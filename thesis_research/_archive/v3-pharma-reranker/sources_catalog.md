...
| 5 | **ChPL↔Ulotka alignment rate** | **≥90% par** | both endpoints HTTP 200 + valid PDF (URL-e z XML attributes `charakterystyka`/`ulotka`); supplemental HTTP `Last-Modified` ±1 day jeśli header dostępny | 100 random `productID` |
...
| URL feed (page) | `https://rejestry.ezdrowie.gov.pl/registry/rpl` (daily XML/XLSX/CSV) |
| URL XML feed | `https://rejestry.ezdrowie.gov.pl/api/rpl/medicinal-products/public-pl-report/6.0.0/overall.xml` |
| URL XSD schema | `https://rejestry.ezdrowie.gov.pl/api/rpl/medicinal-products/public-pl-report/6.0.0/xml-schema-definition.xsd` |
| URL XLSX (alt) | `https://rejestry.ezdrowie.gov.pl/api/rpl/medicinal-products/public-pl-report/get-xlsx` |
| URL ChPL/Ulotka per-product | URL bezpośrednio w XML attributes `charakterystyka`/`ulotka` na `<produktLeczniczy>` (NIE template) |
| Content | Charakterystyka Produktu Leczniczego — standaryzowane 10 sekcji per lek |
| Skala | **22,750 produktów w URPL universe** (Iter. 0a probe 2026-05-16, snapshot stanNaDzien=2026-05-15) → po filter (complete pair: ChPL+Ulotka URL + ATC code) **12,973 effective scrape pool** (57% retention; excluded: 9,469 no-URL EU-centralized/legacy + 1,148 no-ATC herbal/special) |
...
**Stratified sampling algorithm (900 leków z URPL effective pool ~13k complete-pair products, per Iter. 0a probe 2026-05-16):**
...
| URL per-product | URL w XML attribute `ulotka` na `<produktLeczniczy>`, format `https://rejestry.ezdrowie.gov.pl/api/rpl/medicinal-products/{N}/leaflet` (N = sequential, ≠ `id` attribute) |
| Content | Laypersonowski odpowiednik ChPL — QRD-aligned 6 sekcji |
| Skala | ~13k complete pair products (1:1 z ChPL, after URPL universe filter — per Iter. 0a probe 2026-05-16) |
...
1. URL-e `charakterystyka` i `ulotka` w XML attributes oba zwracają HTTP 200 + valid PDF (paired URLs generowane razem przez tę samą decyzję URPL — semantic equivalence by construction)
2. **Date alignment** — per-product `data_modyfikacji` **NIE istnieje** w XML feed (tylko snapshot-level `stanNaDzien` na root `<produktyLecznicze>`). Alignment date check via HTTP `Last-Modified` header z PDF response (jeśli serwer ustawia); fallback na "both endpoints OK" (acceptable bo paired URLs strukturalnie aligned przez URPL workflow)
