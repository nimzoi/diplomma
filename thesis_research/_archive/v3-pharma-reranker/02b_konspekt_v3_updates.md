...
1. **Long tail polish-specific linguistic patterns wokół międzynarodowej terminologii.** Substancje czynne (DCI / INN) i kody ATC są międzynarodowe — BGE-M3 i polish-reranker base znają je z pretraining corpus (np. *sertralina* PL ≈ *sertraline* EN ≈ *sertralinum* INN; ATC `N06AB06` identyczny cross-language). ALE polskie ChPL i Ulotki używają **polish-specific patternów** które off-the-shelf modele słabo handlują:
   - **Polska fleksja gramatyczna** substancji czynnych przez 7 przypadków (*sertralina / sertralinę / sertraliny / sertralinie / sertraliną*) — reranker musi rozpoznać że to ten sam lek niezależnie od formy
   - **Polski elastyczny szyk + złożona fleksja** w zapytaniach (*„Czy sertralinę można brać z alkoholem"* vs *„Czy z alkoholem można brać sertralinę"*)
   - **Regulatorowa frazeologia PL** specyficzna dla ChPL/Ulotek: *„Należy zachować szczególną ostrożność..."*, *„Nie zaleca się stosowania..."*, *„Decyzję o podawaniu leku..."*
   - **Polskie ramy regulacyjne** bez angielskiego ekwiwalentu: *„ryczałt 30%"*, *„katalog leków refundowanych"*, programy lekowe B.xx, AOTMiT rekomendacje
   - **Polskie dosage instructions**: *„dwa razy dziennie po posiłku"*, *„nie kruszyć tabletek"*, *„rano i wieczorem"*

   Domain adaptation rerankera adresuje **te polish-specific patterns**, NIE międzynarodową terminologię (która już jest znana). Realnie nowy element to **cross-register transformation** (RQ5, DEC-002) — pacjent pisze *„zaszkodzi"*, ChPL mówi *„działanie niepożądane"* — to jest unique to Polish PIL/SPC pair, brak w global literature (Grabowski 2017 = EN-PL, NIE intra-PL).
...
## II.5 Gradio demo + chat product framing (DODAJE do v3 FINAL II.5)

W ramach Task 05 GUI requirement (institutional Task 05 sekcja "Deployment and Infrastructure Architecture") + konspektu v3 FINAL II.5.X (FastAPI + Gradio API), system jest demonstrowany przez **Gradio UI z 3 zakładkami**:

| Zakładka | Funkcjonalność | Cel |
|---|---|---|
| **Demo** | User pisze pytanie po polsku → BGE-M3 retrieve top-50 → polish-reranker rerank → Bielik gen via SGLang → answer + cited sources (productID + section reference) | End-to-end RAG flow demonstration |
| **Compare** | Side-by-side: baseline reranker vs dotrenowany (cykl N). Per-query top-10 passages + scores. Wizualne pokazanie improvement. | Empiryczna ilustracja H1 (RQ1) |
| **Annotate** | Author oznacza ranking quality (5-Likert per top-K passage). Eksport do JSONL. | Ad-hoc eval beyond test set; możliwa augmentacja training data |

**Framing kluczowe — ABSOLUTNIE NIE clinical advisor:**

- ✅ User **widzi czat** (Task 05 GUI requirement met)
- ✅ Promotor **demoluje pipeline end-to-end** (research artefact validation)
- ✅ Każda odpowiedź pokazuje **cited sources** (productID, ChPL/Ulotka section) — transparentnie pokazuje co reranker zwrócił
- ❌ **NIE walidujemy** medycznej correctness (konspekt II.2.2 explicit wyłączenie)
- ❌ **NIE deployujemy** jako clinical advisor

**Disclaimers w UI (mandatory):**
> *„System NIE udziela porad medycznych ani farmaceutycznych. Wyłącznie demonstracja pipeline'u retrievalu w polskojęzycznym RAG dla domeny farmakologii klinicznej. Odpowiedzi mogą zawierać błędy — zawsze konsultuj farmaceutę lub lekarza dla decyzji klinicznych."*

Banner zawsze widoczny na każdej zakładce. Brak banner → spec violation.

**Tech stack zakładek (z konspektu v3 FINAL II.5 + 02b stack updates):**
- Frontend: Gradio (Python-native, no JS required)
- Inference: FastAPI proxy → SGLang serving (Bielik) + TEI serving (BGE-M3 + reranker)
- Persistence (Annotate tab): SQLite local + DVC export

**Cross-reference w R5 (Architektura):** sekcja "User-facing demo" obowiązkowa, z Figurą 5.8 (Gradio UI mock-up screenshot) + disclaimer text verbatim quote.

---

## II.5 Cache-Augmented Generation w inference layer (DODAJE do v3 FINAL II.5)

W ramach pivotów v1→v3 CAG (Cache-Augmented Generation) został de-prioritized jako **research question** — NIE jest 6. RQ. ALE pozostaje aktywnym **engineering layer** w warstwie inference, foregrounded explicit w R5 z dwóch powodów: (1) realna optymalizacja prod-grade serving (typowe pharma queries powtarzają się — *„skutki uboczne X"* zadawane wielokrotnie różnymi formami), (2) MLOps-friendly extension architektury bez scope creep na RQ.

### Stack — dwa cache layers (już w konspekt v3 II.11 stack, foregrounded w R5)

| Layer | Technology | Cel | Latency impact (expected) |
|---|---|---|---|
| **Semantic cache** | **GPTCache** w Redis Stack | Hit dla identycznych/podobnych queries (cosine sim z BGE-M3 embedding ≥0.95) → response z cache, skip retrieval+generation | ~50-80% latency reduction dla cache hit; expected hit ratio w pharma ~30-40% post-warmup |
| **KV-prefix cache** | **LMCache** w SGLang serving | KV-cache reuse między requestami z tym samym prefiksem (system prompt + few-shot examples są identyczne dla wszystkich queries, ~500 shared tokens) | ~30-50% first-token latency reduction dla shared prefix |

### Inference flow z CAG (Figura 5.9 — Cache-augmented inference detail)

```
User query
   ↓
[GPTCache lookup: cosine sim ≥0.95 z prev queries?]
   ├── HIT  → response z cache (skip retrieval + generation, ~50-80% latency reduction)
   └── MISS ↓
[BGE-M3 retrieve top-50 z Qdrant]
   ↓
[polish-reranker rerank top-50 → top-10]
   ↓
[SGLang serving (Bielik 11B v3) z LMCache prefix reuse (~500 shared tokens)]
   ↓
[Response generation]
   ↓
[Write (query, response) → GPTCache (TTL 24h)]
   ↓
Return to user
```

### Architectural decisions (do R5 sekcja 5.X "CAG layer")

- **Semantic cache threshold:** cosine sim ≥0.95 (high precision — zwracamy z cache tylko gdy bardzo pewni). False positive cost: out-of-date response → mitigated przez TTL.
- **TTL semantic cache:** 24h (pharma content updates są na poziomie dni/tygodni; 24h to safe margin)
- **LMCache prefix:** system prompt + few-shot examples (statyczne dla wszystkich queries — ~500 tokens shared)
- **Cache eviction:** LRU dla GPTCache (size limit 10k entries); LMCache: per-session capacity limit z SGLang config
- **Cache invalidation hook:** gdy corpus snapshot zmienia się (DVC version bump po Iter 1/2/3), full GPTCache flush (model output mógł się zmienić)

### Secondary metric w R7 (Wyniki — NIE nowy RQ)

R7 raportuje (jako **operational metric** w nowej sekcji 7.X "CAG layer effectiveness", **NIE research question** — nie liczy się jako RQ6):
- **p95 latency cold cache vs warm cache** (po 1000 random queries z eval set) — pokazuje real-world serving improvement
- **Cache hit rate** post-warmup (po 1h synthetic traffic na eval set) — pokazuje cache effectiveness w pharma domain
- **Quality preservation:** czy cached responses mają taką samą jakość jak fresh (sample 100 cached vs 100 fresh, manual review lub `<judge_model>` faithfulness comparison)

### Literatura (już w R2 cytowane)

- Chan et al. 2025 *Don't Do RAG: When Cache-Augmented Generation is All You Need* (WWW '25) — z konspekt v3 FINAL II.11 reference
- Jin et al. 2024 *RAGCache: Efficient Knowledge Caching for Retrieval-Augmented Generation* (arXiv:2404.12457) — z konspekt v3 FINAL II.11 reference

### Framing dla R5

CAG **NIE jest research question** — to **engineering layer**. To pozwala:
- Pokazać architectural breadth (post-v1 CAG nie zniknął, tylko reframed jako engineering)
- Spełnić MLOps mindset promotora (production-ready inference = real concern)
- NIE inflować scope na 5 → 6 RQ (which was rejected w v1→v2→v3 pivotach)

**Cross-reference w R5 (Architektura):** sekcja "Cache-Augmented Generation layer" obowiązkowa, z **Figurą 5.9 (Cache-augmented inference flow detail)** + Tabelą 5.X (cache decisions + thresholds).
...
### II.13.10. Multi-turn chat evaluation (NEW, 2026-05-16)

**Implicit chat handling — IN scope.** Final RAG pipeline używa LlamaIndex `ChatEngine` z builtin conversation memory (rephrase follow-up jako standalone query przed retrieval). Reranker pipeline strukturalnie niezmieniony — uczy się single query→passage relevance, conversation handling jest w upstream Query Construction layer. Demo zakładka Gradio działa multi-turn out-of-the-box.

**Multi-turn formal evaluation — OUT scope (future work).** 200 par gold standard są single-turn; RAGAS suite (context_precision, context_recall, faithfulness, answer_relevancy) NIE testuje multi-turn coherence. Future work: dodanie ~20 par multi-turn do eval set, RAGAS extension lub własne metryki conversation coherence (e.g. context carry-over rate, follow-up resolution accuracy). Estymowany koszt: ~3-5 dni — odsuwany do post-podstawowych-RQ scope.

**Decyzja podjęta:** 2026-05-16 — implicit chat działa via LlamaIndex; formalne mierzenie multi-turn coherence dopisane jako future work w R8 po zamknięciu 5 RQ.
