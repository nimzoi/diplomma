...
#### Sekcja 7.X: CAG layer effectiveness (operational metrics — NIE RQ)

Per 02b § II.5 CAG inference layer — CAG jest **engineering layer**, nie research question. Raportujemy w R7 jako **secondary operational metrics**, separate od 5 RQ:

- **Tabela 7.X.1:** p95 latency cold cache vs warm cache (po 1000 random queries z eval set)
  - Expected: ~50-80% reduction dla cache hit, ~30-50% dla LMCache prefix reuse
- **Tabela 7.X.2:** Cache hit rate post-warmup (po 1h synthetic traffic) — pokazuje cache effectiveness w pharma domain
  - Expected: 30-40% semantic cache hit ratio post-warmup
- **Figura 7.X.1:** Latency distribution cold vs warm (boxplot lub histogram)
- **Quality preservation check:** 100 cached vs 100 fresh responses — `<judge_model>` faithfulness comparison (lub manual review). Acceptance: cached quality ≥ fresh quality - 2pp.
- **Cache invalidation discussion:** how DVC corpus snapshot bumps trigger GPTCache flush; impact on warmup time.

**Discussion (4-part jak inne sekcje):**
1. **What table shows** — numerical latency improvement + cache hit rate
2. **Co to znaczy** — production-ready serving feasibility w pharma domain
3. **Relate to literature** — Chan 2025 CAG (✓ paradigm), Jin 2024 RAGCache (✓ architecture)
4. **Limitations** — synthetic traffic ≠ real users; cache invalidation on corpus updates może być costly; quality drift przy long TTL.

**Framing dla R8 (limitations + future work):**
- Limitation: CAG eval na synthetic traffic, NIE real production users
- Future work: real-time cache adaptation (ML-based hit prediction, dynamic TTL), evaluation na real prod traffic
